...
void UART0_Transmit(uint8_t data)
{
	...
	while(!(UART0->S1 & UART0_S1_TDRE_MASK)){} // cat bufferul e plin(tdre), astept 
				UART0->D = data; // continutul bufferului populat
...
void UART0_Init(uint32_t baud_rate)
{
	...
	PORTA->PCR[1] = ~PORT_PCR_MUX_MASK; // ~ <=> negare
	...
	UART0->C2 &= ~((UART0_C2_RE_MASK) | (UART0_C2_TE_MASK));  // da totul 0; dc o facem daca tot da 0? in caz ca a fost mai devreme un init si C2 a fost modif
																														// deci fortam in C, bitii TE SI RE sa fie 0, restul 1 ,adica dezactivam pe RE si TE
	...
	uint16_t sbr = (uint16_t)((DEFAULT_SYSTEM_CLOCK)/(baud_rate * (osr+1))); // formula din reffereance manual pag 726
	uint8_t temp = UART0->BDH & ~(UART0_BDH_SBR(0x1F)); // o val interm din BDH
	UART0->BDH = temp | UART0_BDH_SBR(((sbr & 0x1F00)>> 8)); // baud rate Low si High
	...
	UART0->C2 |= UART0_C2_TIE(0); // nu fac nimic
	UART0->C2 |= UART0_C2_TCIE(0); // nu fac nimic
	
	//Activare intreruperi la receptie
	UART0->C2 |= UART0_C2_RIE(1); 
	
	UART0->C2 |= ((UART_C2_RE_MASK) | (UART_C2_TE_MASK)); // reactivam RE SI TE
...
void UART0_IRQHandler(void) {
		...
		if(UART0->S1 & UART0_S1_RDRF_MASK) { //verific daca registriul S1(registru de stare) are date in el(am receptionat date)
			c = UART0->D;	// daca am in S1, atucni datele efective le gasesc in registrul D, luam continutul datelor, salvez in c(am octet)
			
			if (c=='?')	// daca primesc ? afisez
				print_buffer=1;
			else 
			{
				buffer[write] = c;
				write++;
				if (write==32) 	// se reseteaza dupa ce am primit mai mult de un octet
					full=1;		
				write = write % 32;
			}
		}
